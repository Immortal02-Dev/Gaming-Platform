...
import { fetchPreMatch, fetchEventMarkets } from "@/app/sports/sportsApi";
                      ...
                      <div className="match-market">
                        <div className="match-marketItem">
                          ...
                          {primary.selections.length === 3 ? (
                            // 3-way: home | draw (center) | away
                            <>
                              <button
                                className="match-marketBtn"
                                disabled={primary.selections[0].suspended}
                              >
                                <strong className="match-marketName">
                                  <p>{primary.selections[0].name}</p>
                                </strong>
                                <span
                                  className="match-marketOdds"
                                  data-odds={primary.selections[0].odds}
                                >
                                  {primary.selections[0].suspended
                                    ? "중단"
                                    : fmt(primary.selections[0].odds)}
                                </span>
                              </button>
                              <div className="match-marketVersus">
                                {fmt(primary.selections[1].odds)}
                              </div>
                              <button
                                className="match-marketBtn"
                                disabled={primary.selections[2].suspended}
                              >
                                <strong className="match-marketName">
                                  <p>{primary.selections[2].name}</p>
                                </strong>
                                <span
                                  className="match-marketOdds"
                                  data-odds={primary.selections[2].odds}
                                >
                                  {primary.selections[2].suspended
                                    ? "중단"
                                    : fmt(primary.selections[2].odds)}
                                </span>
                              </button>
                            </>
                          ) : (
                            ...
                            <>
                              ...
                              <button
                                className="match-marketBtn"
                                disabled={primary.selections[1]?.suspended}
                              >
                                <strong className="match-marketName">
                                  <p>{primary.selections[1]?.name}</p>
                                </strong>
                                <span
                                  className="match-marketOdds"
                                  data-odds={primary.selections[1]?.odds}
                                >
...
function DetailPanel({ eventId }: { eventId: string | null }) {
  ...
  const [collapsedMarkets, setCollapsedMarkets] = useState<Set<string>>(
    new Set(),
  );
  ...
  const visibleMarkets: Market[] = useMemo(() => {
    ...
    if (activeFilter === "other")
      return event.markets.filter((m) => m.title !== "Match Winner");
...
function PreMatchSportsPageContent() {
  ...
  useEffect(() => {
    if (filtered.length > 0) {
      if (
        selectedId === null ||
        !filtered.some((m) => m.event_id === selectedId)
      ) {
                      ...
                      <div className="match-tabItem active">
                        <div className="match-tabInner">
                          <button className="match-tabBtn active">
                            <span className="match-tabText">24시간이내</span>
                          </button>
                        </div>
                      </div>
