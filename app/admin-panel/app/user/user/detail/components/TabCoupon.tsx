...
export default function TabCoupon({ userIdx }: TabCouponProps) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "1":
        return <span className="badge bg-success">사용</span>;
      case "2":
        return <span className="badge bg-danger">취소</span>;
      case "3":
        return <span className="badge bg-secondary">만료</span>;
      default:
        return status;
    }
  };
          ...
          <div className="col-3">
            <div className="bg-light p-2 border rounded">
              ...
              <div className="fw-bold text-warning">
                {summary.waitAmount?.toLocaleString()}
              </div>
            </div>
          </div>
          <div className="col-3">
            <div className="bg-light p-2 border rounded">
              ...
              <div className="fw-bold text-success">
                {summary.useAmount?.toLocaleString()}
              </div>
            </div>
          </div>
          <div className="col-3">
          ...
          <div className="col-3">
            <div className="bg-light p-2 border rounded">
              ...
              <div className="fw-bold text-secondary">
                {summary.expireAmount?.toLocaleString()}
              </div>
            </div>
          </div>
        </div>
              ...
              {loading ? (
                ...
                logs.map((log: CouponLog, index: number) => (
                  <tr key={log.id}>
                    ...
                    <td>
                      {log.register_date
                        ? new Date(log.register_date).toLocaleString()
                        : "-"}
                    </td>
                    <td>
                      {log.use_date
                        ? new Date(log.use_date).toLocaleString()
                        : "-"}
                    </td>
                    <td>
                      {log.expire_date
                        ? new Date(log.expire_date).toLocaleDateString()
                        : "-"}
                    </td>
                  </tr>
                ))
