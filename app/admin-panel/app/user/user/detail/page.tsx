...
export function UserDetailPageInner({
  userIdxProp,
  tabTypeProp,
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/admin/user/detail?userIdx=${userIdx}`,
        {
          credentials: "include",
      ...
      <div className="panel panel-inverse" style={{ height: "inherit" }}>
        ...
        <div className="panel-body bg-gray-200">
          ...
          <ul className="nav nav-tabs mb-3" style={{ overflowX: "auto" }}>
            {tabs.map((tab) => (
              <li className="nav-item" key={tab.id}>
                <button
                  className={`nav-link bg-primary  ${activeTab === tab.id ? "active fw-bold text-white" : "bg-gray-200 text-black"}`}
          ...
          <div className="tab-content panel p-3 rounded mb-0 bg-white">
            {activeTab === "basic" && (
              <TabBasicInfo user={userData} onSaved={fetchUserData} />
            )}
            {activeTab === "money_action" && <TabMoneyAction user={userData} />}
            {activeTab === "charge_exchange" && (
              <TabChargeExchange userIdx={userIdx} />
            )}
            ...
            {activeTab === "message" && (
              <TabMessage
                userIdx={userIdx}
                onWriteMessage={openMessageWritePopup}
              />
            )}
