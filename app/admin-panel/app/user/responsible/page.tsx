...
export default function ResponsiblePage() {
  ...
  const [activeTab, setActiveTab] = useState<"exclusions" | "limits">(
    "exclusions",
  );
  ...
  const fetchExclusions = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/admin/responsible/exclusions?${params.toString()}`,
        {
          credentials: "include",
        },
      );
  ...
  const fetchLimits = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/admin/responsible/limits?${params.toString()}`,
        {
          credentials: "include",
        },
      );
  ...
  const handleCancelExclusion = async (id: number) => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/admin/responsible/exclusions/${id}`,
        {
          method: "DELETE",
          credentials: "include",
        },
    ...
    <Layout>
      ...
      <h1 className="page-header">
        <a href="/user/responsible">
          <i className="fa fa-hand-holding-heart me-2"></i>건전한 게임 관리
                  ...
                  {loading ? (
                    ...
                    exclusions.map((e) => (
                      <tr key={e.id}>
                        ...
                        <td>
                          <span
                            className={`badge ${e.type === "exclusion" ? "bg-danger" : "bg-warning text-dark"}`}
                          >
                        ...
                        <td
                          className={
                            isExpired(e.end_date) ? "text-muted" : "text-danger"
                          }
                        >
                        ...
                        >
                          {e.reason || (
                            <span className="text-muted small">사유 없음</span>
                          )}
                        </td>
                        <td>
                        ...
                        <td>
                          {!isExpired(e.end_date) && (
                            <button
                              ...
                              onClick={() => handleCancelExclusion(e.id)}
                            >
                              해제
                            </button>
                          )}
                    ...
                    limits.map((l) => (
                      <tr key={l.id}>
                        ...
                        <td className="text-danger">
                          {formatCurrency(l.daily_loss_limit)}
                        </td>
                        <td className="text-danger">
                          {formatCurrency(l.weekly_loss_limit)}
                        </td>
                        <td className="text-danger">
                          {formatCurrency(l.monthly_loss_limit)}
                        </td>
                        <td className="text-primary">
                          {formatCurrency(l.daily_deposit_limit)}
                        </td>
