...
export default function UserLoginPage() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          const roleInfo = errorData.details
            ? ` (현재 역할: ${errorData.details.userRole}, 필요 역할: ${errorData.details.requiredRoles?.join(", ") || "admin"})`
            : "";
          throw new Error(
            `권한이 부족합니다. 관리자 계정으로 로그인해주세요.${roleInfo}`,
          );
        ...
        throw new Error(
          errorData.error ||
            `Failed to fetch login sessions: ${response.status}`,
        );
      }
  ...
  const fetchLoginSessionsRef = useRef(fetchLoginSessions);

  useEffect(() => {
    fetchLoginSessionsRef.current = fetchLoginSessions;
  });

  useEffect(() => {
    fetchLoginSessionsRef.current();
              ...
              <tbody id="loginList">
                {loading ? (
                  ...
                  sessions.map((session, index) => (
                    <tr
                      key={session.id}
                      data-roleidx={session.roleType === "partner" ? "3" : "4"}
                    >
                      <td>{index + 1}</td>
                      <td>
                        {getRoleLabel(session.roleType, session.roleLevel)}
                      </td>
                      <td>
                        <a
                          className=""
                      ...
                      <td className="p-1">
                        <a
                          ...
                          onClick={(e) => {
                            ...
                            window.messageWrite?.(
                              session.user_id,
                              session.roleType === "member" ? 9 : undefined,
                            );
                          }}
                        >
                        ...
                        <a
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            window.userDetail?.(session.user_id, 8);
                          }}
                        >
