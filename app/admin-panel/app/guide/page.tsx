...
export default function GuidePage() {
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingItem
        ? `${BACKEND_URL}/api/admin/guides/${editingItem.id}`
        : `${BACKEND_URL}/api/admin/guides`;
  ...
  const deleteItem = async (id: number) => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/admin/guides/${id}`, {
        method: "DELETE",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      });
  ...
  const updateActive = async (id: number, next: boolean) => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/admin/guides/${id}/active`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  const updateOrder = async (id: number, nextOrder: number) => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/admin/guides/${id}/order`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <Layout>
      <h1 className="page-header">
        <a href="/guide">
          <i className="fa fa-info-circle me-2"></i>가이드 관리
        </a>
        <small></small>
      </h1>
          ...
          <div className="d-flex bg-white p-2 ">
            ...
            <div className="ms-auto">
              <button className="btn btn-primary btn-sm" onClick={openCreate}>
                <i className="fas fa-edit me-2"></i>가이드 등록
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="panel panel-inverse mb-0">
                <div className="panel-heading ui-draggable-handle">
                  <h4 className="panel-title">
                    ...
                    <span>{editingItem ? "가이드 수정" : "가이드 등록"}</span>
                  </h4>
